...
void TrocarPeca(dados *Jogador,int r,peca **PecasRestantes,int *qPecasRestantes,char jogada[30]){
  ...
  do{
    ...
    for(int j = 0; j<p ; j++){
      if(pecas[j][0] >= 'A' && pecas[j][0] <= 'F' && pecas[j][1] >= '1' && pecas[j][1] <= '6'){
        ...
        if(flag == 0){
          printf("Voce nao possui uma dessas pecas,tente denovo!\n");
          return;
        }
      } else {
        printf("Uma das pecas sao invalidas, tente denovo!\n");
        return;
      }
    }
  }while(flag != 1);
