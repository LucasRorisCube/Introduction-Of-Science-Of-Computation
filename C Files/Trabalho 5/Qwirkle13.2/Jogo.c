...
void TrocarPeca(dados *Jogador,int r,peca **PecasRestantes,int *qPecasRestantes){
  ...
  for(int i = 0;i<18;i++){
      for(int j = 0;j<6;j++){
        if(PecasRestantes[i][j].letra == '0' && PecasRestantes[i][j].numero == '0'){
          PecasRestantes[i][j].letra = Jogador[r].pecas[PecaTrocada].letra;
          PecasRestantes[i][j].numero = Jogador[r].pecas[PecaTrocada].numero;
          flag = 1;
          break;
        }
      }
  ...
  for(int i = PecaTrocada;i<Jogador[r].qpecas-1;i++){
