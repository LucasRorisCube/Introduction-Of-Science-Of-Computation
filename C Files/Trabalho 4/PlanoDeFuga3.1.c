...
void Inicializar(FILE *Usuarios,dados *usuario){
  for(int k = 0;k<NumUsuarios(Usuarios);k++){
    fgets(usuario[k].nick,40,Usuarios);
    fgets(usuario[k].NomeArquivo,40,Usuarios);
    int r = 0;
    ...
    setbuf(stdin,NULL);
    fgets(usuario[k].nome,40,usuario[k].Ausuario);
    ///
    fscanf(usuario[k].Ausuario,"%d",&usuario[k].NPosts);
    ...
    for(int i = 0;i<usuario[k].NPosts;i++){
      ...
      fgets(usuario[k].Posts[i],130,usuario[k].Ausuario);
      fgets(usuario[k].Posts[i],130,usuario[k].Ausuario);
      printf("Recebi: %s",usuario[k].Posts[i]);
    }
    ///
    fscanf(usuario[k].Ausuario,"%d",&usuario[k].NSeguindo);
    ...
    for(int i = 0;i<usuario[k].NSeguindo;i++){
      setbuf(stdin,NULL);
      fgets(usuario[k].Seguindo[i],40,usuario[k].Ausuario);
    }
    ///
    fscanf(usuario[k].Ausuario,"%d",&usuario[k].NSeguidores);
    ...
    for(int i = 0;i<usuario[k].NSeguidores;i++){
      setbuf(stdin,NULL);
      fgets(usuario[k].Seguidores[i],40,usuario[k].Ausuario);
    }
  }
  ///
}
...
void AdicionarPost(dados *usuario,int r){
  ...
  if(usuario[r].NPosts == 0){
    usuario[r].Posts = malloc(sizeof(char*)*5);
