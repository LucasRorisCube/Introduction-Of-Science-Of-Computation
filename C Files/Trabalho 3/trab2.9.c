...
///////////////////////////
void jogadapc(int m,int p,struct dados jogador[m],int i,char simbolo,int n,char jogo[n][n]){ // em andamento
printf("%c vai jogar\n",simbolo);
  srand((unsigned)time(NULL));
  int l,c;
  int flag = 0;
  for(int j = i;i<m;i++){
    if(verifganhar(n,p,jogo,jogador[j].simbolo,simbolo) == 1){
      return;
    }
  }
  for(int k = 0;k<i;k++){
    if(verifganhar(n,p,jogo,jogador[k].simbolo,simbolo) == 1){
      return;
    }
  }
